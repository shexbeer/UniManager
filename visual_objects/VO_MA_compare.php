...
class VO_MA_compare
{
	...
	{
			$this->UM->tpl->assign("result", $error_occurred);
	...
	function showCompareList($sg, $semester, $compareList, $lehrbeauf, $lb_ma)
	{
		$this->UM->showfooter();
		$this->UM->showheader($this->UM->seite);
		...
		$this->UM->tpl->assign("lehrbeaufList", $lehrbeauf);
		$this->UM->tpl->assign("lb", $lb_ma);
